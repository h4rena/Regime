      ...
      <div class="auth-form-card">
        ...
        <form class="auth-form" method="POST" action="<?= base_url('/inscription') ?>" data-ajax-form="register-step1">
          ...
          <div class="form-group">
            ...
            <input id="nom" type="text" name="nom" class="form-input" data-field="nom" value="<?= esc($old['nom'] ?? '') ?>" required />
            <small class="field-error" data-error-for="nom"><?= esc($errors['nom'] ?? '') ?></small>
          </div>

          <div class="form-group">
            <label class="form-label" for="prenom">Prenom</label>
            <input id="prenom" type="text" name="prenom" class="form-input" data-field="prenom" value="<?= esc($old['prenom'] ?? '') ?>" required />
            <small class="field-error" data-error-for="prenom"><?= esc($errors['prenom'] ?? '') ?></small>
          </div>

          <div class="form-group">
            <label class="form-label" for="email">Email</label>
            <input id="email" type="email" name="email" class="form-input" data-field="email" value="<?= esc($old['email'] ?? '') ?>" required />
            <small class="field-error" data-error-for="email"><?= esc($errors['email'] ?? '') ?></small>
          </div>

          <div class="form-group">
            <label class="form-label" for="date_naissance">Date de naissance</label>
            <input id="date_naissance" type="date" name="date_naissance" class="form-input" data-field="date_naissance" value="<?= esc($old['date_naissance'] ?? '') ?>" required />
            <small class="field-error" data-error-for="date_naissance"><?= esc($errors['date_naissance'] ?? '') ?></small>
          </div>

          <div class="form-group">
            <label class="form-label">Genre (checkbox)</label>
            <div class="checkbox-grid" data-single-check-group>
              <?php foreach (($genres ?? []) as $genre): ?>
                <?php $checked = ((string) ($old['genre_id'] ?? '') === (string) $genre['id']) ? 'checked' : ''; ?>
                <label class="checkbox-card">
                  <input type="checkbox" name="genre_id" value="<?= esc($genre['id']) ?>" <?= $checked ?> data-field="genre_id" data-single-check />
                  <span><?= esc($genre['nom']) ?></span>
                </label>
              <?php endforeach; ?>
            </div>
            <small class="field-error" data-error-for="genre_id"><?= esc($errors['genre_id'] ?? '') ?></small>
          </div>
